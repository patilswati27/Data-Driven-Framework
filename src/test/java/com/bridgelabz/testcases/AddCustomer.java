...
/**
 * @author swati
 * Purpose:To get data from file and to account
 *
 */
public class AddCustomer extends TestBase {
	...
	public void AddCustomer(Hashtable<String, String> data) throws InterruptedException {
		 ...
		 * //if(!TestUtil.isTestRunnable("AddCustomer", excel)) throw new
		 ...
		 * postCode,String alerttext) throws InterruptedException { //
		 * log.info("Add test");
		 ...
		 * firstName);
		 * driver.findElement(By.cssSelector(OR.getProperty("lastname_Css"))).sendKeys(
		 * lastName);
		 ...
		 * Alert a=wait.until(ExpectedConditions.alertIsPresent());//to handle alert pop
		 * up Assert.assertTrue(a.getText().contains("alerttext")); Thread.sleep(3000);
